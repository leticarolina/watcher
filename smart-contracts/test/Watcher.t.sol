...
contract WatcherTest is Test {
    ...
    // Ledger signer test fixture — a known private key so signatures can be produced with vm.sign.
    uint256 constant LEDGER_KEY = 0xBEEF;
    ...
    function setUp() public {
        ...
        ledgerSignerAddr = vm.addr(LEDGER_KEY);
    }
    ...
    /// @dev Signs the executeWithdraw digest (user, amount, block.chainid) with the given private key.
    function _signLedger(uint256 privateKey, address user, uint256 amount) internal view returns (bytes memory) {
    ...
    */
    ...
    function test_withdraw_insufficientBalanceReverts() public deposited(leti, 1 ether) {
        ...
        vm.expectRevert(abi.encodeWithSelector(Watcher.InsufficientBalance.selector, 1 ether));
    ...
    */
    ...
    function test_requestSafeAddressChange_success() public deposited(leti, 1 ether) {
        ...
        emit Watcher.SafeAddressChangeRequested(shai, eve, block.timestamp + 24 hours);
        vault.requestSafeAddressChange(eve);
    ...
    function test_withdrawToSafe_reverts_ifBalanceZero() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Watcher.InsufficientBalance.selector, 0));
        vault.withdrawToSafe(1 ether);
    ...
    */
    ...
    function test_registerLedgerSigner_reverts_ifAlreadySet() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
    ...
    // ---- requestLedgerSignerChange ----

    function test_requestLedgerSignerChange_success() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);

        uint256 expectedUnlock = block.timestamp + vault.SAFE_ADDRESS_CHANGE_DELAY();
        ...
        assertEq(vault.pendingLedgerSigner(leti), shai);
        assertEq(vault.ledgerSignerChangeUnlockTime(leti), expectedUnlock);
    }
    ...
    function test_requestLedgerSignerChange_reverts_ifPendingExists() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(shai);
    ...
    function test_requestLedgerSignerChange_reverts_ifLocked() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.emergencyLock(1 hours);
    ...
    function test_requestLedgerSignerChange_toZeroAddress_requestsRemoval() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(address(0));
        vm.stopPrank();

        assertEq(vault.pendingLedgerSigner(leti), address(0));
        assertGt(vault.ledgerSignerChangeUnlockTime(leti), 0);
    }

    // ---- confirmLedgerSignerChange ----

    function test_confirmLedgerSignerChange_success() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(shai);
        ...
        assertEq(vault.ledgerSigner(leti), shai);
        assertEq(vault.pendingLedgerSigner(leti), address(0));
        assertEq(vault.ledgerSignerChangeUnlockTime(leti), 0);
    }
    ...
    function test_confirmLedgerSignerChange_reverts_ifDelayNotOver() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(shai);
    ...
    function test_confirmLedgerSignerChange_reverts_ifLocked() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(shai);
    ...
    function test_confirmLedgerSignerChange_removal_deletesLedgerSigner() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(address(0));
        ...
        assertEq(vault.ledgerSigner(leti), address(0));
    }

    // ---- cancelLedgerSignerChange ----

    function test_cancelLedgerSignerChange_success() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(shai);
    ...
    function test_cancelLedgerSignerChange_allowedDuringLock() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(shai);
        vault.emergencyLock(1 hours);

        vault.cancelLedgerSignerChange(); // should not revert while locked
    ...
    // ---- executeWithdraw signature verification ----

    function test_executeWithdraw_withValidLedgerSignature_succeeds() public deposited(leti, 1 ether) {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);

        uint256 withdrawAmount = 0.8 ether; // >60% → risky, goes pending
        vault.withdraw(withdrawAmount);

        vm.warp(block.timestamp + 12 hours);

        bytes memory sig = _signLedger(LEDGER_KEY, leti, withdrawAmount);
    ...
    function test_executeWithdraw_withInvalidSignature_reverts() public deposited(leti, 1 ether) {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
    ...
    function test_executeWithdraw_withNoLedgerSigner_emptyBytes_succeeds() public deposited(leti, 1 ether) {
        vm.startPrank(leti);
        ...
        uint256 beforeBalance = leti.balance;
        vault.executeWithdraw(""); // no ledgerSigner registered, verification skipped
        vm.stopPrank();

        assertEq(leti.balance, beforeBalance + withdrawAmount);
    }

    function test_executeWithdraw_withWrongAmount_inSignature_reverts() public deposited(leti, 1 ether) {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
    ...
    function test_executeWithdraw_withWrongChainId_inSignature_reverts() public deposited(leti, 1 ether) {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        ...
        // Signed digest uses a different chainid than the current one
        bytes32 hash = keccak256(abi.encodePacked(leti, withdrawAmount, block.chainid + 1));
        bytes32 digest = MessageHashUtils.toEthSignedMessageHash(hash);
    ...
    // ---- getLedgerSignerState ----

    function test_getLedgerSignerState_noSigner() public view {
        (address current, address pending, uint256 remaining) = vault.getLedgerSignerState(leti);
    ...
    function test_getLedgerSignerState_withSigner() public {
        vm.prank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);

        (address current, address pending, uint256 remaining) = vault.getLedgerSignerState(leti);

        assertEq(current, ledgerSignerAddr);
        assertEq(pending, address(0));
        assertEq(remaining, 0);
    }

    function test_getLedgerSignerState_withPendingChange() public {
        vm.startPrank(leti);
        vault.registerLedgerSigner(ledgerSignerAddr);
        vault.requestLedgerSignerChange(shai);
        vm.stopPrank();
        ...
        assertEq(current, ledgerSignerAddr);
        assertEq(pending, shai);
        assertEq(remaining, vault.SAFE_ADDRESS_CHANGE_DELAY());
    }
}
